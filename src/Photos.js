...
import { FETCH_RECOMMENDED_PHOTOS_QUERY } from "./solutions/exercise4";
          ...
          <Toolbar>
            <Typography color="inherit">
              {this.state.selectedPhotos.length > 4
                ? "Businesses recommended based on photo selections"
                : "Select 5 photos to seed business recommendations"}
            </Typography>
            {this.state.selectedPhotos.length > 4 ? (
              <Button onClick={this.clearSelectedPhotos}>Reset</Button>
            ) : null}
          </Toolbar>
        </AppBar>
        {this.state.selectedPhotos.length > 4 ? (
